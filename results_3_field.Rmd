...
#histogram of number of landslides surveyed
fig_slip_num <- ggplot(field_data, aes(x = slip_number)) +
  geom_histogram(binwidth = 1, fill = "grey", color = "black", na.rm = TRUE) +
  labs(x = "Number of Landslides Surveyed",
       y = "Studies") +
  theme_light() +
  theme(
...
groups_field_prop <- list(
  ...
  prop_erosion = c("soil movement", "erosion rate", "landslide activity", "sediment runoff", "rill prevalence", "rill proximity")
)
...
colnames(prop_f_matrix) = c("Topography", "Weather", "Size", "Seed Source Prox.", "Disturbance Prox.", "Prior Disturbance", "Zone", "Soil - Chemical", "Soil - Physical", "Erosion")

# Create UpSet plot
fig_field_prop <- upset(prop_f_matrix, mainbar.y.label = "Property Intersections", sets.x.label = "Studies",
      sets = c("Topography", "Weather", "Size", "Seed Source Prox.", "Disturbance Prox.", "Prior Disturbance", "Zone", "Soil - Chemical", "Soil - Physical", "Erosion"), mb.ratio = c(0.55, 0.45), order.by = "freq", text.scale = 1.7, point.size = 2.2)
```
...
#plot size histogram
fig_plot_size <- ggplot(field_data, aes(x = plot_size)) +
  geom_histogram(binwidth = 1, fill = "grey", color = "black", na.rm = TRUE) +
  scale_x_continuous(trans = 'sqrt') +  # Breaks at squared values
  labs(x = expression("Plot Size (m"^2*")"),
       y = "Studies") +
  theme_light() +
  theme(
    axis.title = element_text(size = 14),
...
## Chronosequence & Remeasurement

```{r time plots}
# Remeasurement tidying

field_data$remeasure_visits <- as.numeric(field_data$remeasure_visits)

field_data <- field_data %>%
  mutate(remeasure_time = remeasure_end - remeasure_start,
         remeasure_frequency = remeasure_visits / (remeasure_time + 1)) #add one as first visit to site was included in remeasure_visits

# Remeasurement plot

fig_remeasure <- ggplot(field_data, aes(x = remeasure_time, y = (remeasure_frequency))) +
  geom_point(size = 3, alpha = 0.7) +
  #scale_y_continuous(trans='sqrt') +
  labs(
    x = "Remeasurement Duration (years)",
    y = "Remeasurement Frequency (years)",
  ) +
  theme_light() +
  theme(
    axis.title = element_text(size = 14),
...
field_data$chrono_age <- as.numeric(field_data$chrono_age)

fig_chrono <- ggplot(field_data, aes(x = "", y = chrono_age)) +
  geom_boxplot(alpha = 0.7, outlier.shape = 21, outlier.size = 3) +
    labs(
    x = "Chronosequence Studies",
    y = "Age of Oldest Landslide (years)",
  ) +
  scale_y_continuous(trans="sqrt") +
  theme_light()+
  theme(
...
ggplot(field_data, aes(x = chrono_age)) +
  geom_histogram(binwidth = 5, fill = "#6aaed6", color = "black") +
  labs(x = "Chronosequence Age (years)", y = "Number of Studies") +
  theme_minimal() 

```

```{r time stats summary}
# Remeasurement 
remeasure_studies <- field_data %>%
  filter(grepl("remeasure", time, ignore.case = TRUE))

summary(remeasure_studies$remeasure_time)
summary(remeasure_studies$remeasure_frequency)

# Chrono Plots

chrono_studies <- field_data %>%
  filter(grepl("chronosequence", time, ignore.case = TRUE))

summary(chrono_studies$slip_number)

# Chrono Replicates

table(chrono_studies$chrono_replicate)

# Chrono Age

summary(chrono_studies$chrono_age)
...
```{r figure exporting}
fig_9 <- fig_slip_num / fig_plot_size
ggsave("figs/fig_9.svg", plot = fig_9, width = 7, height = 8)

fig_10 <- fig_remeasure + fig_chrono + plot_layout(widths = c(4, 1.1))
ggsave("figs/fig_10.svg", plot = fig_10, width = 9, height = 5)

pdf("figs/fig_11.pdf", width = 9, height = 5.5)
print(fig_field_prop)
dev.off()
