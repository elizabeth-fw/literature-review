...
screening_sixth <- screen_abstracts(screening_sixth)
...
abstracts_screened <- subset(screening_fifth, select = -(30:1716))
...
keywords_tmp_2 <- tibble(id = 1:378, title = abstracts_selected$title, keywords = abstracts_selected$keywords)
...
authors_tmp <- tibble(id = 1:378, title = abstracts_selected$title, author = abstracts_selected$author)
...
abst_processed <- textProcessor(documents = abstracts_selected$abstract, metadata = abstracts_selected, custompunctuation = c("“", "”", "‘", "£", "≥", "°" , "′", "″"))

abst_vocab <- abst_processed$vocab

# remove results that do not occur frequently - originally 10

plotRemoved(abst_processed$documents, lower.thresh = seq(1, 200, by = 100))

out <- prepDocuments(abst_processed$documents, abst_processed$vocab, abst_processed$meta, lower.thresh = 5)
...
rescreening_first <- screen_abstracts(abst_rescreen)
